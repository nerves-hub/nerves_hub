defmodule NervesHub.UpdateHandler do
  ...
  @callback should_reboot?() :: boolean()
  @callback update_frequency() :: pos_integer()
  @callback reboot_frequency() :: pos_integer()
  ...
  def should_reboot?(handler) do
    case apply(handler, :should_reboot?, []) do
      b when is_boolean(b) ->
        b
  ...
  def update_frequency(handler) do
    case apply(handler, :update_frequency, []) do
      ...
      bad ->
        Logger.error(
          "[NervesHub] #{handler} not rescheduling update. bad return: #{inspect(bad)}"
        )
  ...
  @doc "Returns a timeout in milliseconds for when an reboot should be rescheduled."
  @spec reboot_frequency(module) :: false | pos_integer()
  def reboot_frequency(handler) do
    case apply(handler, :reboot_frequency, []) do
      freq when freq > 0 ->
        freq

      bad ->
        Logger.error(
          "[NervesHub] #{handler} not rescheduling reboot. bad return: #{inspect(bad)}"
        )
